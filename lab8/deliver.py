...
class Controller:
	...
	if True:
		...
		rospy.loginfo('GOAL: %s',(goal_x,goal_y))
	    	dif_x = goal_x - self.cur_x
	    	dif_y = self.cur_y - goal_y
		
		if abs(dif_x) < 60 and abs(dif_y) < 60:
		...
		if abs(correction) > 25:
		    cmd_vel.angular.z = np.sign(correction) * -0.1
	    	...
	    	if abs(dif_y) < 45 and dif_x > 0:
		    g_dir = 3
	    	elif abs(dif_y) < 45 and dif_x < 0:
		    g_dir = 1
	    	elif abs(dif_x) < 45 and dif_y > 0:
		    g_dir = 2
	    	else:
		    g_dir = 0
		...
		rospy.loginfo(movement   / np.pi * 180.0)
		...
		if movement > np.pi:
		    cmd_vel.angular.z = -0.30
                ...
                if abs(movement) < 20.0 * np.pi / 180:
                    self.state = 'move'
...
if __name__ == '__main__':
    ...
    parser.add_argument("middle_node", help="Middle node name", type=str)
    ...
    middle_x = nodes[MIDDLE][0]
    middle_y = nodes[MIDDLE][1]
    end_x = nodes[END][0]
    end_y = nodes[END][1]

    START_P = nodes[START]

    PATH = plan_path((starting_x,starting_y),(middle_x,middle_y))
    PATH2 = plan_path((middle_x,middle_y),(end_x,end_y))
    PATH2.pop()
